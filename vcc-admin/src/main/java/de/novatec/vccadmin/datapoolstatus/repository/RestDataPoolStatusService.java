...
public class RestDataPoolStatusService implements DataPoolStatusService {
    ...
    @Override
    public TargetStatus saveTargetStatus(TargetStatus targetStatus) {
        LOGGER.info("Trying to post to " + TARGETSTATUS_ENDPOINT);
        ResponseEntity<TargetStatus> responseEntity = restTemplate.postForEntity(TARGETSTATUS_ENDPOINT, targetStatus, TargetStatus.class);
        return responseEntity.getBody();
    }

    @Override
    public void updateTargetStatus(TargetStatus targetStatus) {
        String url = TARGETSTATUS_ENDPOINT + "/" + targetStatus.getDataPoolId();
        LOGGER.info("calling put on " + url + " for update targetstatus " + targetStatus);
        restTemplate.put(url, targetStatus);
    }
